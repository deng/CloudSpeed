...
class LeftSideBar extends PureComponent {
  ...
  getSelectedMenuKeys = () => {
    let selectMenu = getMeunMatchKeys(this.props.flatMenu, this.props.location.pathname + this.props.location.search)[0];
    if (selectMenu) {
      return selectMenu ? [selectMenu.path] : [];
    }
    const pathname = this.props.location.pathname;
    selectMenu = getMeunMatchKeys(this.props.flatMenu, pathname)[0];
    return selectMenu ? [selectMenu.path] : [];
  };
